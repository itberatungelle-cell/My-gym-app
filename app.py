    ...
    with st.form("gym_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Speichern"):
            ...
            try:
                ...
                if r.ok:
                    st.success(f"Gespeichert! Bitte Seite neu laden.")
                    st.cache_data.clear()
...
if not df.empty:
    ...
    # --- NEU/ZURÜCK: AUSLASTUNG PRO WOCHENTAG (BALKEN) ---
    st.subheader("📊 Durchschnittliche Auslastung pro Tag")
    if not filtered_df.empty:
        bar_data = filtered_df.groupby("Wochentag", observed=True)["Auslastung"].mean().reset_index()
        bars = alt.Chart(bar_data).mark_bar(cornerRadiusTopLeft=5, cornerRadiusTopRight=5).encode(
            ...
            y=alt.Y('Auslastung:Q', title="Schnitt (1-10)", scale=alt.Scale(domain=[0, 10])),
